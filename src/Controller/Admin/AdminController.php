...
class AdminController extends AbstractController
{
    ...
    public function orderHasArrived(ObjectManager $objectManager)
    {
       ...
       if ( isset( $_POST['arrived']) && $workflows->can($box, 'order_received'))
       {
            $workflows->apply($box, 'order_received');
            $objectManager->flush();
       }
       elseif (isset( $_POST['validated']) && $workflows->can($box, 'order_approved') )
       {
     ...
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function orderIsReadyToGo(ObjectManager $objectManager)
    {
        $box =  $objectManager->getRepository(Box::class)->findActiveBox();
        $box = $box[0];
